...
### **Synopsis**
  ...
  1. **Problem Statement**: The objective of this project is to explore the Kaggle Movies dataset and get some interesting results like which countries produce most movies, profitability analysis, specific kind of movies that are produced most, most produced genres etc.



  2. **Solution Overview**: Post the data cleaning, univariate and multi-variate exploration of the data helped us achieve some of the above mentioned statistical goals. A variety of packages were used for this exercise, which are explained in the later sections. Each analysis has been explained along with its codes. The codes are hidden from the report, but all of them are being executed in the process.
  ...
  3. **Insights**: Some very interesting insights were obtained from this analysis:
    ...
    * English is the language in which most movies are made. (US and EU industry mostly focuses on English)
...
*Assumption: The results we get might be biased due to the movies that are collected in the dataset. This might not represent a true picture of the population of movies. Hoewver, it is a good guess to make that those results will be similar for a bigger dataset as well.*
...
The first part of data cleaning involves removal of spurious characters (?) from movie title, genre and plot_keywords columns. This could have come up during data scrapping from the net.
...
Third part of data cleaning is the currency columns. The "Budget" and "Gross" (revenue) columns for a few countries were not converted to USD while compiling the data. Part of I have done manually so that I can work with it in the later parts.

Lastly I created a profit_flag column, which is 1 if the movie is profitable (Revenue > Budget) and 0 otherwise.

I used basic R functionallity for most of the part, as extended functions from packages like 'stringr' were not required for these tasks.
...
After the data cleaning, I once again check the number of rows and columns, as shown in the code below. The results show 4,917 unique movies. I consider the data to be clean enough for my analysis.
...
Below is the table containing the variable names, data types and a short description.
...
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.width=9, fig.height=8}
...
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.width=9}
...
I wanted to study the relationship between IMDB score, revenue and budget of the movie. So I plotted a 3D scatter plot using the "plotly" package. This is an interactive graph and hence we can comfortably see the relationship between the three variables. Plotly has an advantage of being interactive and so we can rotate and see all the observations. Red signifies loss for the movie while blue signifies profit:
...
Then I wanted to see which countries do well in terms of the movie IMDB Score. I combined this analysis with the overall profitability of the movies produced in the country (total revenue - total budget). Scatter plot below shows this analysis. Size if the bubble indicates the number of movies produced in the country while colour shows protability (Green is profit, red is loss):
